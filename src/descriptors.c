...
void parse_iso639_language_descriptor(const unsigned char *buf,
				      struct service *s)
{
...
void parse_S2_satellite_delivery_system_descriptor(const unsigned char *buf,
						   void *dummy)
{
	hd(buf);
						...
						inversion)
{
	...
	switch ((buf[8] & 0x60) >> 5) {
	case 0:
		t->polarization = POLARIZATION_HORIZONTAL;
		break;
	case 1:
		t->polarization = POLARIZATION_VERTICAL;
		break;
	case 2:
		t->polarization = POLARIZATION_CIRCULAR_LEFT;
		break;
	case 3:
		t->polarization = POLARIZATION_CIRCULAR_RIGHT;
		break;
	default:
	...
	t->symbolrate =
	    10 * bcd32_to_cpu(buf[9], buf[10], buf[11], buf[12] & 0xF0);
	//FEC_inner 4 bslbf
	switch (buf[12] & 0x0F) {
	case 1:
		t->coderate = FEC_1_2;
	...
	if ((t->modulation == PSK_8) ||
	    (t->rolloff == ROLLOFF_25) ||
	    (t->rolloff == ROLLOFF_20) ||
	    (t->coderate == FEC_9_10) || (t->coderate == FEC_3_5)) {
		verbose
		    ("\t%s: fixing broken NIT, setting modulation_system to DVB-S2.\n",
		     __FUNCTION__);
		t->delsys = SYS_DVBS2;
	}
}
					    ...
					    fe_spectral_inversion_t inversion)
{
	...
	case 3:
		...
		break;
	case 4:
		t->modulation = QAM_128;
		break;
	case 5:
		t->modulation = QAM_256;
		break;
	default:
...
//descriptor_tag_extension = buf[2];                                                                 // descriptor_tag_extension     8 uimsbf
	...
	case 0:
		...
		break;
	case 1:
		t->C2_tuning_frequency_type = C2_SYSTEM_CENTER_FREQUENCY;
		break;
	case 2:
		t->C2_tuning_frequency_type =
		    INITIAL_TUNING_FOR_STATIC_DATA_SLICE;
		break;
		//case 3: reserved_for_future_use
	default:
		t->C2_tuning_frequency_type = DATA_SLICE_TUNING_FREQUENCY;	// This is the default option for C2 systems
	}
	switch ((*bp & 0x38) >> 3) {	// active_OFDM_symbol_duration  3 uimsbf
	case 0:
		t->active_OFDM_symbol_duration = FFT_4K_8MHZ;
		break;		// 448µsec    (4k FFT mode for 8MHz CATV systems)
	case 1:
		t->active_OFDM_symbol_duration = FFT_4K_6MHZ;
		break;		// 597,33µsec (4k FFT mode for 6MHz CATV systems)
		//case 2 ... 7: reserved_for_future_use                                                           //
	default:
		t->active_OFDM_symbol_duration = FFT_4K_8MHZ;	// defaulting to here to 8MHz CATV systems, as nothing better found so far.
	}
	switch (*bp & 0x07) {	// guard_interval               3 bslbf
	case 0:
		t->guard = GUARD_INTERVAL_1_128;
		break;		//
	case 1:
		t->guard = GUARD_INTERVAL_1_64;
		break;		// not defined in linux dvb api, see extended_frontend.h
		//case 2 ... 7: reserved_for_future_use                                                           //
	default:
						  ...
						  inversion)
{
	...
	case 0:
		...
		break;		//non-hierarchical, native interleaver
	case 1:
		t->hierarchy = HIERARCHY_1;
		break;		//alpha = 1, native interleaver
	case 2:
		t->hierarchy = HIERARCHY_2;
		break;		//alpha = 2, native interleaver
	case 3:
		t->hierarchy = HIERARCHY_4;
		break;		//alpha = 4, native interleaver
	case 4:
		t->hierarchy = HIERARCHY_NONE;
		break;		//non-hierarchical, in-depth interleaver
	case 5:
		t->hierarchy = HIERARCHY_1;
		break;		//alpha = 1, in-depth interleaver
	case 6:
		t->hierarchy = HIERARCHY_2;
		break;		//alpha = 2, in-depth interleaver
	case 7:
		t->hierarchy = HIERARCHY_4;
		break;		//alpha = 4, in-depth interleaver
	default:
		t->hierarchy = HIERARCHY_NONE;
	}
	...
	case 0:
		t->coderate_LP = FEC_1_2;
		break;
	case 1:
		t->coderate_LP = FEC_2_3;
		break;
	case 2:
		t->coderate_LP = FEC_3_4;
		break;
	case 3:
		t->coderate_LP = FEC_5_6;
		break;
	case 4:
		t->coderate_LP = FEC_7_8;
		break;
	default:
	...
	case 0:
		t->guard = GUARD_INTERVAL_1_32;
		break;
	case 1:
		t->guard = GUARD_INTERVAL_1_16;
		break;
	case 2:
		t->guard = GUARD_INTERVAL_1_8;
		break;
	case 3:
		t->guard = GUARD_INTERVAL_1_4;
		break;
	default:;
	}
	switch ((buf[8] >> 1) & 0x3) {	// transmission_mode 2 bslbf
	case 0:
		t->transmission = TRANSMISSION_MODE_2K;
		break;
	case 1:
		t->transmission = TRANSMISSION_MODE_8K;
		break;
	case 2:
		t->transmission = TRANSMISSION_MODE_4K;
		break;
	default:
...
//descriptor_tag_extension = buf[2];                                                                 // descriptor_tag_extension     8 uimsbf
	...
	if ((t->extended_info = (descriptor_length > 4))) {	// has extension
		...
		case 4:
			...
			break;
		case 5:
			t->guard = GUARD_INTERVAL_19_128;
			break;
		case 6:
			t->guard = GUARD_INTERVAL_19_256;
			break;	//       111 reserved for future use
		default:
			t->guard = GUARD_INTERVAL_AUTO;
		}
		...
		case 2:
			...
			break;
		case 3:
			t->transmission = TRANSMISSION_MODE_1K;
			break;
		case 4:
			t->transmission = TRANSMISSION_MODE_16K;
			break;
		case 5:
			t->transmission = TRANSMISSION_MODE_32K;
			break;
		default:
			t->transmission = TRANSMISSION_MODE_AUTO;	//       110 to 111 reserved for future use
		}
					    ...
					    *buf)
{
	...
	for (i = 0; i < d.number_elements; i++) {
		struct ATSC_service_location_element e =
		    read_ATSC_service_location_element(b);
		...
		for (j = 0; j < num_seg; j++) {
			...
			case uncompressed_string:
				if (s->service_name)
					free(s->service_name);
				s->service_name =
				    malloc(num_bytes * sizeof(char) + 1);
				memcpy(s->service_name, &b[3], num_bytes);
				s->service_name[num_bytes] = '\0';
				break;
			default:
				warning
				    ("compressed strings are not supported yet\n");
				break;
			}
			b += 3 + num_bytes;
		}
...
const char *network_id_desc(uint16_t network_id)
{
	switch (network_id) {
	case 0x0000:
		return "reserved network id";
	case 0x0001 ... 0x2000:
		return "unique satellite network id";
	case 0x2001 ... 0x3000:
		return "unique terrestrial network id";
	case 0x3001 ... 0x3600:
		return "reusable terrestrial network id";
	case 0x3601 ... 0xA000:
		return "terrestrial network id (future use)";
	case 0xA001 ... 0xB000:
		return "reuseable cable network id";
	case 0xB001 ... 0xF000:
		return "cable network id (future use)";
	case 0xF001 ... 0xFEBF:
		return "unique cable network id";
	case 0xFEC0 ... 0xFF00:
		return "common interface betwork id";
	default:
		return "private use network id";
	}
}
					 ...
					 fe_spectral_inversion_t inversion)
{
	...
	case 0:
		diversity_mode = DIVERSITY_OFF;
		break;		//       no  no  no  no
		// case 1..7:                                                                                      //       reserved for future use
	case 8:
		diversity_mode = DIVERSITY_PATS;
		break;		//       yes no  no  no
		// case 9..12:                                                                                     //       reserved for future use
	case 13:
		diversity_mode = DIVERSITY_PATS_FEC_LINK;
		break;		//       yes yes no  yes
	case 14:
		diversity_mode = DIVERSITY_PATS_FEC_PHY;
		break;		//       yes yes yes no
	case 15:
		diversity_mode = DIVERSITY_PATS_FEC_PHY_LINK;
		break;		//       yes yes yes yes
	default:
		diversity_mode = DIVERSITY_AUTO;	//
	}			//
	...
	while (descriptor_length > 0) {	//  for (i=0; i<N; i++){
		...
		bp++;
		descriptor_length--;	//      Reserved 5 bslbf
		if (modulation_loop[n_modulations].modulation_type == MODULATION_TYPE_SH_B_TDM) {	//      if (modulation_type == 0) {
			...
			default:
				modulation_loop[n_modulations].roll_off = ROLLOFF_AUTO;	//
			}	//
			...
			case 2:
				...
				//case 3: reserved for future use                                                           //
			default:
				modulation_loop[n_modulations].modulation_mode = QPSK;	//                     //NOTE: no valid default here.
			}	//
			...
			case 0:
				...
				break;	//
			case 1:
				modulation_loop[n_modulations].bandwidth = 8000000;	//                     // 00001 8   1/8  62/9    56/9    52/9
				...
				break;	//
			case 2:
				modulation_loop[n_modulations].bandwidth = 8000000;	//                     // 00010 8   1/16 116/17  108/17  100/17
				...
				break;	//
			case 3:
				modulation_loop[n_modulations].bandwidth = 8000000;	//                     // 00011 8   1/32 224/33  208/33  64/11
			...
			case 4:
				...
				break;	//
			case 5:
				modulation_loop[n_modulations].bandwidth = 7000000;	//                     // 00101 7   1/8  217/36  49/9    91/18
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_8;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 6:
				modulation_loop[n_modulations].bandwidth = 7000000;	//                     // 00110 7   1/16 203/34  189/34  175/34
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_16;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 7:
				modulation_loop[n_modulations].bandwidth = 7000000;	//                     // 00111 7   1/32 196/33  182/33  56/11
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_32;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 8:
				modulation_loop[n_modulations].bandwidth = 6000000;	//                     // 01000 6   1/4  51/10   24/5    87/20
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_4;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
			...
			case 9:
				...
				break;	//
			case 10:
				modulation_loop[n_modulations].bandwidth = 6000000;	//                     // 01010 6   1/16 87/17   81/17   75/17
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_16;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 11:
				modulation_loop[n_modulations].bandwidth = 6000000;	//                     // 01011 6   1/32 56/11   52/11   48/11
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_32;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 12:
				modulation_loop[n_modulations].bandwidth = 5000000;	//                     // 01100 5   1/4  17/4    4/1     29/8
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_4;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 13:
				modulation_loop[n_modulations].bandwidth = 5000000;	//                     // 01101 5   1/8  155/36  35/9    65/18
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_8;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 14:
				modulation_loop[n_modulations].bandwidth = 5000000;	//                     // 01110 5   1/16 145/34  135/34  125/34
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_16;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 15:
				modulation_loop[n_modulations].bandwidth = 5000000;	//                     // 01111 5   1/32 140/33  130/33  40/11
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_32;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 16:
				modulation_loop[n_modulations].bandwidth = 1712000;	//                     // 10000 1.7 1/4  34/25   32/25   29/25
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_4;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 17:
				modulation_loop[n_modulations].bandwidth = 1712000;	//                     // 10001 1.7 1/8  62/45   56/45   52/45
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_8;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 18:
				modulation_loop[n_modulations].bandwidth = 1712000;	//                     // 10010 1.7 1/16 116/85  108/85  20/17
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_16;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
				...
				break;	//
			case 19:
				modulation_loop[n_modulations].bandwidth = 1712000;	//                     // 10011 1.7 1/32 224/165 208/165 64/55
				modulation_loop[n_modulations].guard = GUARD_INTERVAL_1_32;	//
				switch (modulation_loop[n_modulations].roll_off) {	//
				case ROLLOFF_15:
					modulation_loop
			...
			case 0:
				modulation_loop[n_modulations].bandwidth = 8000000;	//             // 000 8 MHz
			case 1:
				modulation_loop[n_modulations].bandwidth = 7000000;	//             // 001 7 MHz
			case 2:
				modulation_loop[n_modulations].bandwidth = 6000000;	//             // 010 6 MHz
			case 3:
				modulation_loop[n_modulations].bandwidth = 5000000;	//             // 011 5 MHz
			case 4:
				modulation_loop[n_modulations].bandwidth = 1712000;	//             // 100 1,7 MHz
			default:;	//
			}	//
			...
			case 0:	//
			case 1:
				modulation_loop[n_modulations].hierarchy = HIERACHY_NONE;	//
			...
			default:	//
				...
				case 0:
					...
					break;	//
				case 1:
					modulation_loop[n_modulations].transmission = TRANSMISSION_MODE_2K;	//                     // 01 2k
					break;	//
				case 2:
					modulation_loop[n_modulations].transmission = TRANSMISSION_MODE_4K	//                     // 10 4k
					    break;	//
				case 3:
...
int crc_check(const unsigned char *buf, __u16 len)
{
	...
	if (crc == transmitted_crc)
		...
		warning
		    ("received garbage data: crc = 0x%08x; expected crc = 0x%08x\n",
		     crc, transmitted_crc);
		return 0;
	}
}

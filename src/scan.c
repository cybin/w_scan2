...
static void setup_filter(struct section_buf *s, const char *dmx_devname,
			 int pid, int table_id, int table_id_ext, int run_once,
			 int segmented, uint32_t filter_flags);
static void add_filter(struct section_buf *s);
static void copy_fe_params(struct transponder *dest,
...
int is_different_transponder_deep_scan(struct transponder *a,
				       struct transponder *b, int auto_allowed)
{
...
#define IS_DIFFERENT(A, B, _ALLOW_AUTO_, _AUTO_) ( (A != B)  && (! _ALLOW_AUTO_ || (_ALLOW_AUTO_ && (A != _AUTO_) && (B != _AUTO_)) ) )
	...
	case SCAN_TERRESTRIAL:
		if (IS_DIFFERENT
		    (a->modulation, b->modulation, auto_allowed, QAM_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->bandwidth, b->bandwidth, auto_allowed, 8000000))
			return 1;
		if (IS_DIFFERENT
		    (a->coderate, b->coderate, auto_allowed, FEC_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->hierarchy, b->hierarchy, auto_allowed, HIERARCHY_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->coderate_LP, b->coderate_LP, auto_allowed, FEC_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->transmission, b->transmission, auto_allowed,
		     TRANSMISSION_MODE_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->guard, b->guard, auto_allowed, GUARD_INTERVAL_AUTO))
			return 1;
	...
	case SCAN_CABLE:
		...
		if (IS_DIFFERENT
			...
			return 1;
		if (IS_DIFFERENT
		    (a->delsys, b->delsys, auto_allowed, SYS_DVBC_ANNEX_AC))
			return 1;
		if (IS_DIFFERENT(a->plp_id, b->plp_id, auto_allowed, 0))
			return 1;
		if (IS_DIFFERENT(a->system_id, b->system_id, auto_allowed, 0))
	...
	case SCAN_SATELLITE:
		...
		if (IS_DIFFERENT
			...
			return 1;
		if (IS_DIFFERENT
		    (a->rolloff, b->rolloff, auto_allowed, ROLLOFF_AUTO))
			return 1;
		if (IS_DIFFERENT
		    (a->modulation, b->modulation, auto_allowed, QPSK))
			return 1;
		return 0;
	default:
...
static void copy_transponder(struct transponder *dest,
			     struct transponder *source)
{
	struct service *s, *sd;
	struct cell *p;
	...
	if (source->network_name != NULL) {
		dest->network_name =
		    (char *)calloc(strlen(source->network_name) + 1, 1);
	...
	if (source->network_name != NULL) {
		if (dest->network_name != NULL)
			free(dest->network_name);
		dest->network_name =
		    (char *)malloc(strlen(source->network_name) + 1);
		memcpy(dest->network_name, source->network_name,
		       strlen(source->network_name));
	...
	if (source->network_change.num_networks > 0) {
		int i;
		dest->network_change.num_networks =
		    source->network_change.num_networks;
		dest->network_change.network =
		    calloc(source->network_change.num_networks,
			   sizeof(changed_network_t));
		for (i = 0; i < source->network_change.num_networks; i++) {
			dest->network_change.network[i] =
			    source->network_change.network[i];
			dest->network_change.network[i].loop =
			    calloc(source->network_change.network[i].
				   num_changes, sizeof(network_change_loop_t));
			memcpy(&dest->network_change.network[i].loop,
			       &source->network_change.network[i].loop,
			       source->network_change.network[i].num_changes *
			       sizeof(network_change_loop_t));
		}
	} else
...
static int is_known_initial_transponder(struct transponder *tn,
					int auto_allowed)
{
	struct transponder *t;
...
void print_transponder(char *dest, struct transponder *t)
{
	...
	case SCAN_SATELLITE:
		sprintf(dest,
			"%-2s f = %d kHz %s SR = %5d %4s 0,%s %5s  (%u:%u:%u)",
			 ...
			 uint16_t section_length, uint16_t service_id)
{
	...
	while (section_length > 0) {
		...
		case iso_iec_13818_1_private_data:
			...
			if (find_descriptor
				...
				moreverbose("  SUBTITLING: PID %d\n",
					    elementary_pid);
				s->subtitling_pid[s->subtitling_num++] =
				    elementary_pid;
				break;
			} else
			    if (find_descriptor
		...
		case iso_iec_13818_1_11172_1_auxiliary:
			moreverbose
			    ("  ITU-T Rec. H.222.0 | ISO/IEC 13818-1/11172-1 auxiliary : PID %d\n",
			     elementary_pid);
			break;
		case iso_iec_13818_6_type_a_multiproto_encaps:
			moreverbose
			    ("  ISO/IEC 13818-6 Multiprotocol encapsulation    : PID %d\n",
			     elementary_pid);
			break;
		...
		case iso_iec_13818_6_type_c:	//DSM-CC Stream Descriptors
			moreverbose
			    ("  ISO/IEC 13818-6 Stream Descriptors : PID %d\n",
			     elementary_pid);
			break;
		case iso_iec_13818_6_type_d:	//DSM-CC Sections (any type, including private data)
			moreverbose
			    ("  ISO/IEC 13818-6 Sections (any type, including private data) : PID %d\n",
			     elementary_pid);
			break;
		...
		case iso_iec_13818_7_audio_w_ADTS_transp:
			moreverbose
			    ("  ADTS Audio Stream (usually AAC) : PID %d (stream type 0x%x)\n",
			     elementary_pid, buf[0]);
		...
		case iso_iec_14496_1_packet_stream_in_PES:
			moreverbose
			    ("  ISO/IEC 14496-1 SL-packetized stream or FlexMux stream carried in PES packets : PID 0x%04x\n",
			     elementary_pid);
			break;
		case iso_iec_14496_1_packet_stream_in_14996:
			moreverbose
			    ("  ISO/IEC 14496-1 SL-packetized stream or FlexMux stream carried in ISO/IEC 14496 sections : PID 0x%04x\n",
			     elementary_pid);
			break;
		case iso_iec_13818_6_synced_download_protocol:
			moreverbose
			    ("  ISO/IEC 13818-6 DSM-CC synchronized download protocol : PID 0x%04x\n",
			     elementary_pid);
			break;
		case metadata_in_PES:
			moreverbose
			    ("  Metadata carried in PES packets using the Metadata Access Unit Wrapper : PID 0x%04x\n",
			     elementary_pid);
			break;
		case metadata_in_metadata_sections:
			moreverbose
			    ("  Metadata carried in metadata_sections : PID 0x%04x\n",
			     elementary_pid);
			break;
		case metadata_in_iso_iec_13818_6_data_carous:
			moreverbose
			    ("  Metadata carried in ISO/IEC 13818-6 (DSM-CC) Data Carousel : PID 0x%04x\n",
			     elementary_pid);
			break;
		case metadata_in_iso_iec_13818_6_obj_carous:
			moreverbose
			    ("  Metadata carried in ISO/IEC 13818-6 (DSM-CC) Object Carousel : PID 0x%04x\n",
			     elementary_pid);
			break;
		case metadata_in_iso_iec_13818_6_synced_dl:
			moreverbose
			    ("  Metadata carried in ISO/IEC 13818-6 Synchronized Download Protocol using the Metadata Access Unit Wrapper : PID 0x%04x\n",
			     elementary_pid);
			break;
		case iso_iec_13818_11_IPMP_stream:
			moreverbose
			    ("  IPMP stream (defined in ISO/IEC 13818-11, MPEG-2 IPMP) : PID 0x%04x\n",
			     elementary_pid);
			break;
		case iso_iec_14496_10_AVC_video_stream:
			moreverbose
			    ("  AVC Video stream, ITU-T Rec. H.264 | ISO/IEC 14496-10 : PID %d (stream type 0x%x)\n",
			     elementary_pid, buf[0]);
			if (s->video_pid == 0) {
				s->video_pid = elementary_pid;
				s->video_stream_type = buf[0];
		...
		case iso_iec_23008_2_H265_video_hevc_stream:
			moreverbose
			    ("  HEVC Video stream, ITU-T Rec. H.265 | ISO/IEC 23008-1 : PID %d (stream type 0x%x)\n",
			     elementary_pid, buf[0]);
			if (s->video_pid == 0) {
				s->video_pid = elementary_pid;
				s->video_stream_type = buf[0];
		...
		case atsc_a_52b_ac3:
			moreverbose
			    ("  AC-3 Audio per ATSC A/52B : PID %d (stream type 0x%x)\n",
			     elementary_pid, buf[0]);
			if (s->ac3_num < AC3_CHAN_MAX) {
				s->ac3_pid[s->ac3_num] = elementary_pid;
				s->ac3_stream_type[s->ac3_num] = buf[0];
	...
	for (i = 1; i < s->audio_num; i++)
		tmp +=
		    sprintf(tmp, ", %d (%.4s)", s->audio_pid[i],
			    s->audio_lang[i]);

	debug
	    ("tsid=%d sid=%d: %s -- %s, pmt_pid 0x%04x, vpid 0x%04x, apid %s\n",
	     s->transport_stream_id, s->service_id, s->provider_name,
			 ...
			 uint16_t network_id, uint32_t section_flags)
{
	...
	while (section_length > 6) {
		...
		if (section_length < descriptors_loop_len + 4) {
			warning
			    ("section too short: transport_stream_id %u, original_network_id %u, "
			 ...
			 uint16_t section_length, uint16_t transport_stream_id)
{
	...
	while (section_length > 4) {
		...
		if (section_length < descriptors_loop_len
			...
			warning
			    ("section too short: service_id == 0x%02x, section_length == %i, "
			     "descriptors_loop_len == %i\n", service_id,
			     section_length, descriptors_loop_len);
			break;
		}
...
em_static void parse_psip_descriptors(struct service *s,
			      ...
			      uint16_t transport_stream_id)
{
	...
	for (i = 0; i < num_channels_in_section; i++) {
		...
		s->logical_channel_number =
		    ch.major_channel_number << 10 | ch.minor_channel_number;
		...
		info(" Channel number: %d:%d. Name: '%s'\n",
		     ch.major_channel_number, ch.minor_channel_number,
		     s->service_name);
...
//program_info_length = ((buf[10] & 0x0f) << 8) | buf[11];
	...
	if (s->segmented && s->table_id_ext != -1
	...
	if (!get_bit(s->section_done, section_number)) {
		...
		verbosedebug
		    ("pid %d (0x%02x), tid %d (0x%02x), table_id_ext %d (0x%04x), "
		     "section_number %i, last_section_number %i, version %i\n",
		     s->pid, s->pid, table_id, table_id, table_id_ext,
		     table_id_ext, section_number, last_section_number,
...
static int read_sections(struct section_buf *s)
{
	 ...
	 */
	if (((count = read(s->fd, s->buf, sizeof(s->buf))) < 0)
	    && errno == EOVERFLOW)
		...
		if (done || time(NULL) > s->start_time + s->timeout) {
			if (s->run_once) {
				if (done)
					...
					switch (s->table_id) {
					case TABLE_PAT:
						info("%sPAT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_CAT:
						info("%sCAT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_PMT:
						info("%sPMT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_TSDT:
						info("%sTSDT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_NIT_ACT:
						info("%sNIT(actual )after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_NIT_OTH:
						verbose
						    ("%sNIT(other) after %lld seconds\n",
						     intro,
						     (long long)s->timeout);
						break;	// not always available.
					case TABLE_SDT_ACT:
						info("%sSDT(actual) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_SDT_OTH:
						info("%sSDT(other) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_BAT:
						info("%sBAT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_EIT_ACT:
						info("%sEIT(actual) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_EIT_OTH:
						info("%sEIT(other) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_TDT:
						info("%sTDT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_RST:
						info("%sRST after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_TOT:
						info("%sTOT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_AIT:
						info("%sAIT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_CST:
						info("%sCST after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_RCT:
						info("%sRCT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_CIT:
						info("%sCIT after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_VCT_TERR:
						info("%sVCT(terr) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					case TABLE_VCT_CABLE:
						info("%sVCT(cable) after %lld seconds\n", intro, (long long)s->timeout);
						break;
					default:
...
static int set_frontend(int frontend_fd, struct transponder *t)
{
	...
	case SCAN_SATELLITE:
		...
		if (scr_config.user_frequency > 0) {
			...
			intermediate_freq =
			    abs(t->frequency - this_lnb.low_val);
			em_lnb(0, 0, this_lnb.low_val);
		}
		em_polarization(t->polarization);

		if ((intermediate_freq < fe_info.frequency_min)
		    || (intermediate_freq > fe_info.frequency_max)) {
		...
		if ((t->symbolrate < fe_info.symbol_rate_min)
		    || (t->symbolrate > fe_info.symbol_rate_max)) {
		...
		if (sat_list[this_channellist].rotor_position > -1) {	// rotate DiSEqC rotor to correct orbital position
			...
			if (rotate_rotor(frontend_fd, &this_rotor_pos,
					 sat_list
					 [this_channellist].rotor_position,
					 t->polarization ==
	...
	case SCAN_TERRCABLE_ATSC:
		if ((t->frequency < fe_info.frequency_min)
		    || (t->frequency > fe_info.frequency_max)) {
...
#define set_cmd_sequence(_cmd, _data)   cmds[sequence_len].cmd = _cmd; \
		...
		case SCAN_TERRESTRIAL:
			...
			set_cmd_sequence(DTV_MODULATION, t->modulation);
			set_cmd_sequence(DTV_TRANSMISSION_MODE,
					 t->transmission);
		...
		if (t->other_frequency_flag && ((t->cells)->count > 0)) {
			while (i < (t->cells)->count) {
				...
				if ((test != NULL)
					...
					if (tune_to_transponder(frontend_fd, t)
					    == 0)
						return 0;
...
static int dvbc_symbolrate(int index)
{
	...
	case 0:
		return 6900000;	// 8MHz, 6.900MSymbol/s is mostly used for 8MHz
	case 1:
		return 6875000;	// 8MHz, 6.875MSymbol/s also used quite often for 8MHz
	case 2:
		return 6956500;	// 8MHz
	case 3:
		return 6956000;	// 8MHz
	...
	case 10:
		return 6086000;	// 8MHz, 7MHz, sort 7MHz descending by probability
	case 11:
		return 5900000;	// 8MHz, 7MHz
	case 12:
		return 5483000;	// 8MHz, 7MHz

		// 6MHz, Rolloff 0.15 -> 6000000 / 1.15 -> symbolrate <= 5217391,30
	case 13:
		return 5217000;	// 6MHz, 7MHz, 8MHz, sort 6MHz descending by probability
	case 14:
		return 5156000;	// 6MHz, 7MHz, 8MHz
	case 15:
		return 5000000;	// 6MHz, 7MHz, 8MHz
	case 16:
		return 4000000;	// 6MHz, 7MHz, 8MHz
	case 17:
		return 3450000;	// 6MHz, 7MHz, 8MHz
...
static bool initial_table_lookup(int frontend_fd)
{
	...
	setup_filter(&s, demux_devname, PID_PAT, TABLE_PAT, -1, 1, 0,
...
static int initial_tune(int frontend_fd, int tuning_data)
{
	...
	if (tuning_data <= 0) {
		...
		case SCAN_CABLE:
			...
			if (dvbc_symbolrate(dvbc_symbolrate_min) >
			    max_dvbc_srate(freq_step(0, this_channellist))) {
		...
		case SCAN_SATELLITE:
			...
			break;
		default:
			warning("unsupported delivery system %d.\n",
				flags.scantype);
		}
...
static void scan_tp_dvb(void)
{
	...
	current_tp->network_PID = PID_NIT_ST;
	setup_filter(&s[0], demux_devname, PID_PAT, TABLE_PAT, -1, 1, 0,
		     SECTION_FLAG_INITIAL);
	add_filter(&s[0]);
	EMUL(em_readfilters, &result)
	    do {
...
int device_is_preferred(int caps, const char *frontend_name, uint16_t scantype)
{
	...
	else if ((strncmp("Sony CXD2820R", frontend_name, 13) == 0)
		 && (scantype != SCAN_TERRESTRIAL))
...
static void dump_lists(int adapter, int frontend)
{
	...
	if (verbosity > 4)
		bubbleSort(scanned_transponders, cmp_freq_pol);

	for (t = scanned_transponders->first; t; t = t->next) {
		for (s = (t->services)->first; s; s = s->next) {
			if (s->video_pid && !(serv_select & 1))
				continue;	/* no TV services */
			if (!s->video_pid && (s->audio_num || s->ac3_num)
			    && !(serv_select & 2))
...
#define cleanup() cl(country); cl(satellite); cl(initdata); cl(positionfile); cl(codepage);
	...
	while ((opt =
		...
		case 'l':	//satellite lnb type
			...
			if (lnb_decode(optarg, &this_lnb) < 0) {
				cleanup();
				fatal
				    ("LNB decoding failed. Use \"-l ?\" for list.\n");
			}
			/* MHz -> kHz */
			this_lnb.low_val *= 1000;
			this_lnb.high_val *= 1000;
		...
		case 't':	//tuning speed
			...
			if ((flags.tuning_timeout < 1))
				bad_usage(argv[0]);
			if ((flags.tuning_timeout > 3))
				bad_usage(argv[0]);
			break;
		case 'u':	//SCR user definition: <slot>:<user frequency>:<SatPos 'A' or 'B'>(:<Pin>) "N:N:<A|B>(:Pin)"
			{
				...
				case 'B':
					...
					//--------------------------------------------------
				case 'a':
					scr_config.pos = (0U << 6) | (0U << 2);
					scr_config.norm = 2;
					break;
				case 'b':
					scr_config.pos = (0U << 6) | (1U << 2);
					scr_config.norm = 2;
					break;
				case 'c':
					scr_config.pos = (0U << 6) | (2U << 2);
					scr_config.norm = 2;
					break;
				case 'd':
					scr_config.pos = (0U << 6) | (3U << 2);
					scr_config.norm = 2;
					break;
				case 'e':
					scr_config.pos = (1U << 6) | (0U << 2);
					scr_config.norm = 2;
					break;
				case 'f':
					scr_config.pos = (1U << 6) | (1U << 2);
					scr_config.norm = 2;
					break;
				case 'g':
					scr_config.pos = (1U << 6) | (2U << 2);
					scr_config.norm = 2;
					break;
				case 'h':
					scr_config.pos = (1U << 6) | (3U << 2);
					scr_config.norm = 2;
					break;
				case 'i':
					scr_config.pos = (2U << 6) | (0U << 2);
					scr_config.norm = 2;
					break;
				case 'j':
					scr_config.pos = (2U << 6) | (1U << 2);
					scr_config.norm = 2;
					break;
				case 'k':
					scr_config.pos = (2U << 6) | (2U << 2);
					scr_config.norm = 2;
					break;
				case 'l':
					scr_config.pos = (2U << 6) | (3U << 2);
					scr_config.norm = 2;
					break;
				case 'm':
					scr_config.pos = (3U << 6) | (0U << 2);
					scr_config.norm = 2;
					break;
				case 'n':
					scr_config.pos = (3U << 6) | (1U << 2);
					scr_config.norm = 2;
					break;
				case 'o':
					scr_config.pos = (3U << 6) | (2U << 2);
					scr_config.norm = 2;
					break;
				case 'p':
					scr_config.pos = (3U << 6) | (3U << 2);
					scr_config.norm = 2;
					break;
				default:
					fatal
		...
		case 'D':	//DiSEqC committed/uncommitted switch
			...
			case 'u':
				...
				if (uncommitted_switch > 15)
					fatal
					    ("uncommitted switch position needs to be < 16!\n");
				flags.sw_pos =
				    (flags.sw_pos & 0xF) | uncommitted_switch;
				break;
			case 'c':
				committed_switch = i;
				if (committed_switch > 3)
					fatal
					    ("committed switch position needs to be < 4!\n");
				flags.sw_pos =
				    (flags.sw_pos & 0xF0) | committed_switch;
				break;
			default:
				cleanup();
				fatal
		...
		case 'S':	//DVB-C symbolrate index
			dvbc_symbolrate_min = dvbc_symbolrate_max =
			    strtoul(optarg, NULL, 0);
			break;
		case 'T':	//include TV
	...
	if (NULL == initdata) {
		if ((NULL == country) && (scantype != SCAN_SATELLITE)) {
			country =
			    strdup(country_to_short_name(get_user_country()));
	...
	case SCAN_TERRESTRIAL:
		if (country != NULL) {
			...
			choose_country(country, &atsc, &dvb, &scantype,
				       &this_channellist);
			//dvbc: setting qam loop
			if ((modulation_flags & MOD_OVERRIDE_MAX) ==
			    MOD_USE_STANDARD)
				modulation_max =
				    dvbc_qam_max(2, this_channellist);
			if ((modulation_flags & MOD_OVERRIDE_MIN) ==
			    MOD_USE_STANDARD)
				modulation_min =
				    dvbc_qam_min(2, this_channellist);
	...
	case SCAN_SATELLITE:
		...
		if (positionfile != NULL) {
			valid_rotor_data =
			    dvbscan_parse_rotor_positions(positionfile);
			cl(positionfile);
			if (!valid_rotor_data) {
				cleanup();
				fatal
		...
		for (i = 0; i < DVB_ADAPTER_SCAN; i++) {
			for (j = 0; j < 4; j++) {
				snprintf(frontend_devname,
					 ...
					 "/dev/dvb/adapter%i/frontend%i", i, j);
				if ((frontend_fd =
				     open(frontend_devname,
					  fe_open_mode)) < 0) {
					continue;
				}
	...
	EMUL(em_dvbapi, &flags.api_version)
		...
		fatal
		    ("Your DVB driver doesnt support DVB API v5. Please upgrade.\n");

	info("Using DVB API %d.%d\n", flags.api_version >> 8,
	     flags.api_version & 0xFF);

	info("frontend '%s' supports\n", fe_info.name
	     && *fe_info.name ? fe_info.name : "<NULL pointer>");
	...
	case SCAN_TERRESTRIAL:
		...
		if (fe_info.caps & FE_CAN_TRANSMISSION_MODE_AUTO) {
			...
			caps_transmission_mode =
			    dvbt_transmission_mode(5, this_channellist);
		...
		if (fe_info.frequency_min == 0 || fe_info.frequency_max == 0) {
			...
			fe_info.frequency_min = 177500000;
			fe_info.frequency_max = 858000000;
		} else {
			info("FREQ (%.2fMHz ... %.2fMHz)\n",
			     fe_info.frequency_min / 1e6,
			     fe_info.frequency_max / 1e6);
		}
		break;
	case SCAN_CABLE:
		//if (fe_info.caps & FE_CAN_2G_MODULATION) {
		...
		if (fe_info.caps & FE_CAN_QAM_AUTO) {
			...
			for (i = modulation_min; i <= modulation_max; i++)
				info(" %s",
				     modulation_name(dvbc_modulation(i)));
			info(".\n");
			caps_qam = QAM_64;
			flags.qam_no_auto = 1;
		}
		...
		if (fe_info.symbol_rate_min == 0
		    || fe_info.symbol_rate_max == 0) {
			info("This dvb driver is *buggy*: the symbol rate limits are undefined - please report to linuxtv.org\n");
			fe_info.symbol_rate_min = 4000000;
			fe_info.symbol_rate_max = 7000000;
		} else {
			info("SRATE (%.3fMSym/s ... %.3fMSym/s)\n",
			     fe_info.symbol_rate_min / 1e6,
			     fe_info.symbol_rate_max / 1e6);
		}
		break;
	case SCAN_TERRCABLE_ATSC:
		if (fe_info.caps & FE_CAN_INVERSION_AUTO) {
	...
	if (!fe_supports_scan(frontend_fd, scantype, fe_info)
	    && flags.api_version < 0x0505) {
		cleanup();
		fatal
		    ("Frontend '%s' doesnt support your choosen scan type '%s'\n",
